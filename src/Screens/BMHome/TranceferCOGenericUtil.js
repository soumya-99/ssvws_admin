...
export const fetchCONamesBranchWise = async (payload) => {
    ...
    return data.msg.map((el) => ({ label: el.to_co_name, value: el.to_co_id })) ?? [];
}

export const fetchCompletePendingRequestDetails = async (payload) => {
    const { data } = await axios.post(`${url}/transfer_co_view_all_details`, payload);
...
export const TRANSFER_CO_ERROR_MSG = {
    NotFromPendingList: "Sorry system didn't receive any records from pending list. Please go back to the pending list and select one."
}
