...
import DistrictsTable from "../../../../Components/Master/DistrictsTable"
import BlocksTable from "../../../../Components/Master/BlocksTable"
...
function MasterBlocks() {
	...
	const fetchData = async (approvalStat) => {
		setLoading(true)
		...
		await axios
			.get(`${url}/admin/show_all_block`)
			.then((res) => {
				console.log("PPPPPPPPPPPPPPPPPPPP", res?.data)
				if (res?.data?.suc === 1) {
					setData(res?.data?.msg)
					setCopyData(res?.data?.msg)
	...
	const setSearch = (word) => {
		setData(
			copyData?.filter((e) =>
				e?.block_name?.toString()?.toLowerCase().includes(word?.toLowerCase())
			)
		)
	}
